...
\begin{table}[H]
    ...
    \small
    \begin{tabular}{p{2.8cm} p{2.2cm} p{2cm} p{2.2cm} p{2.0cm} p{2.0cm}}
        \toprule
        \textbf{Approche} & \textbf{Cold Start} & \textbf{Dérive temp.} &
        \textbf{Données} & \textbf{Complexité} & \textbf{Guidni~?} \\
        \midrule
        Filtrage collaboratif & Problématique & Mauvaise & Grand historique & Moyenne & Non \\[7pt]
        Basé contenu & Acceptable & Mauvaise & Attributs items & Faible & Partiel \\[7pt]
        Hybride offline & Partiel & Mauvaise & Les deux & Élevée & Non \\[7pt]
...
Dans le système Guidni, dix types d'événements sont suivis avec des valeurs de récompense
...
et que toutes les valeurs sont finies. Tous les 90~jours ou plus, une réinitialisation partielle est
...
plateforme (2000~TND par exemple). Le score d'affinité de tag maximal provient du profil utilisateur. Le
...
une chaîne de dix filtres et ajustements dans un ordre précis après le scoring LinUCB initial.
...
saisonnier élimine les annonces inadaptées à la saison, et le filtre budgétaire supprime les annonces
hors budget. Les étapes suivantes construisent les vecteurs de caractéristiques et appliquent
le scoring LinUCB. Les dernières étapes sont des ajustements post-scoring qui optimisent
la diversité, l'équité, les revenus, avant de finaliser la liste.
...
\begin{enumerate}[leftmargin=2cm]
    ...
    \item \textbf{Filtre saisonnier}~: retirer les annonces inadaptées à la saison courante 
    (par exemple, afficher des articles de plage en plein hiver, ou des équipements de ski en été).
    ...
    \item \textbf{Troncature}~: limiter le résultat final à \texttt{top\_k} éléments (par défaut~10) avant d'être retourné au routeur de recommandation.
\end{enumerate}
...
de l'apprentissage en ligne par bandits contextuels pour le cas d'usage touristique. L'algorithme 
LinUCB a été formalisé mathématiquement, l'ingénierie des caractéristiques par produit de 
Kronecker a été détaillée avec ses 40~tags et son vecteur $\phi$ de dimension~360, et le pipeline 

...
conversationnel basé sur LangGraph avec 18~outils appelables par le LLM, un pipeline RAG
...
longues par résumé automatique à 15~messages. Le cycle de raisonnement LangGraph en quatre
...
partielle pour la dérive conceptuelle ($\lambda = 0.7$ tous les 90~jours). La vérification
...
seule (14~tables miroirs du frontend Prisma/Next.js) et tables gérées par l'IA
(4~tables planificateur + 7~tables recommandation). La réindexation RAG incrémentale via
les listeners d'événements SQLAlchemy maintient l'index vectoriel toujours à jour sans
planification manuelle. Le tracker JavaScript vanilla relie le comportement client aux

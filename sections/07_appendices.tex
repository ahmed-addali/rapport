...
alphabétique strict, définie dans \texttt{app/reco/context.py}, comme le montre le tableau~\ref{tab:tagvocab}. La position de chaque tag
...
réduire les erreurs structurelles détectées par le noeud VALIDATE.
...
\texttt{RecommendationEventUCB}) sont définies dans \texttt{prisma/schema.prisma} mais exploitées
principalement via des requêtes SQL directes dans \texttt{app/reco/db.py} en utilisant
...
du système Guidni dans son état opérationnel et correspondent aux figures~\ref{fig:A1_chat}, \ref{fig:A2_plan}, \ref{fig:A3_health}, \ref{fig:A4_rebuild}, \ref{fig:A5_docs} et \ref{fig:A6_scores}. Elles documentent visuellement les composants
